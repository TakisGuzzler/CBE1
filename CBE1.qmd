...
We then decided to look at the keyness tables for the Democratic and Republican candidates to determine what words might have been influential in their overal sentiments. We found that adjectives where the most statistically significant, with the most frequent verbs, nouns, and other parts of speech being far more generic and neutral.
...
Evidently, most of the most frequent adjectives for both Obama and Harris are neutral. More than that, there appears to be very little overlap between the two sub-corpus in terms of average frequency.
...
From the Trump and Romney discussion, we saw similar pattern of a lack of overlap in high frequency words. However, interestingly the Romney sub corpus more frequently has far more words with a positive sentiment, such as *strong*, *right*, and *peaceful* where many of the most frequent words in the Trump sub corpus have a negative sentiment, such as *bad*, *weak*, *horrible*, *guilty*, and *afraid*.

# Discussion

Most interesting is the difference between the four career politician candidates - Obama, Romney, and Harris - and the non career politician candidate - Trump. Trump would appear far more willing to use negative language. Beyond this, it appears that the sentiment of the candidates language is more informed by the time period than it is by their party. Even though Harris has an overall more positive sentiment when compared to Trump, the two are more negative than their part predecessors - Romney and Obama.
...
ChatGPT was used to help generate the first draft. This primarily took the form of suplementing our lack of knowledge of different forms of sentiment analysis and certain statistical concepts. We asked the LLM about differnet forms of sentiment analysis and what libraries should be used to preforms said analysis in R. We also used the LLM to develop the code used for initially parsing the text into their seperate text files in python. When attempting to fit the Keyness tables on the pdf, I made many queries to the LLM about my problem. In this regard, the LLM was extremely unhepful as none of its suggestions fixed the problem and they ultimetly did not make it to the final product. THe LLM was also used to generate a certain amount of regex lines since I am unfamiliar with the necessary syntax. This ultimately also did not make it to the final report since the analysis in which it was used was deemed unnecessary. Ultimately, the LLM was far more useful in sumplimenting our knowledge of sentiment analysis than it was in our knowledge/implimentation of the R code.
